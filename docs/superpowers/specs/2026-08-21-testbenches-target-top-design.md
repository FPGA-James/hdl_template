...
- **SV** (cocotb-SV, native-sv): the generated SV package (`out/regs/sv/<name>_register_file_axi_lite_pkg.sv`) currently only exposes `hwif_in`/`hwif_out` struct types and bus-width localparams — no address offsets. `` `include ``-ing the C header directly is the simplest fix (SV's preprocessor handles plain C `#define NAME value` macros identically to C, and the generated header contains nothing beyond that), avoiding a new code-generation path. This gets verified for real during implementation (Task covers reading the actual generated header's exact syntax and confirming Verilator/Icarus accept the `` `include ``); if it doesn't work cleanly, the fallback is a small addition to `scripts/gen_regs.py`'s `generate_sv` to also emit a tiny SV localparam package with the same addresses — same computation as the VHDL path, just in SV syntax.
...
- No new `scripts/tests/` pytest coverage is anticipated unless the SV-address-constants fallback (see "Register addressing" above) requires extending `generate_sv` — if that path is taken, it gets the same TDD treatment as every other `gen_regs.py` change this repo already has.
