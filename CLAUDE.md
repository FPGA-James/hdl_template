...
**SV documentation is plainer than VHDL's, by design**: `scripts/gen_filelist.sh` includes SV sources (via Bender's `rtl_sv`/`gen_sv` targets, verified to actually exist on disk — see the script's `bender_flist()` comment for why a naive check isn't enough) so `make html`/`make coverage` work for `TOPLEVEL_HDL=sv` projects too. HDL AutoDoc's `parse_hierarchy.py`/`generate_schematic.py` genuinely support SV (hierarchy discovery, native-Yosys schematics), but `generate_rst.py`'s entity page falls back to a plain `literalinclude` source listing for SV modules instead of the structured port/generic table `.. vhdl:autoentity::` produces — that directive comes from the `sphinx-vhdl` extension and is VHDL-only upstream, a permanent difference in documentation depth, not a bug. An earlier version of `gen_filelist.sh` excluded SV sources entirely to sidestep a real duplicate-module-name collision that occurs when *both* `src/vhdl/` and `src/sv/` exist simultaneously (i.e. only pre-`make init`, when the template ships both languages) — but the exclusion applied unconditionally, breaking the (far more common) post-init single-language case too. Fixed to only take the VHDL branch when `.vhd` files actually exist on disk (always true pre-init, since Bender.yml's file lists aren't pruned by `make init` — only `src/` itself is), falling through to the SV branch otherwise.
